...
async function gradeTextWithAI(opts: {
  ...
  try {
    ...
    return { score: 0, feedback: "AI grading unavailable: server misconfiguration." };
  }

  const system = `You are an expert teaching assistant grading short-answer responses.
You MUST respond with a JSON object containing exactly two fields: "score" (number from 0 to ${opts.points}, decimals allowed) and "feedback" (string with 2-4 sentences of concise, constructive feedback).
...
Student response: ${opts.response || "(blank)"}`;
  ...
  try {
    ...
    if (!content) return { score: 0, feedback: "Unable to parse AI grading output." };
    ...
    console.error("AI grading failed", err?.status ?? err?.message);
    if (err?.status === 429) return { score: 0, feedback: "Grading rate-limited; please retry." };
...
export const submitAssignment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (a.due_at && new Date(a.due_at) < new Date()) {
      throw new Error("This assignment is past its due date and can no longer be submitted.");
    }
